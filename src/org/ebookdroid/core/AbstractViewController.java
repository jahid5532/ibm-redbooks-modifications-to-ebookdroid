...
public abstract class AbstractViewController extends AbstractComponentController<IView> implements IViewController {
    ...
    protected final boolean processLinkTap(final Page page, final PageLink link, final RectF pageBounds, final RectF tapRect) 
    {
    	try
    	{
	        ...
	        //Log.i("linkRect is =====> ",linkRect+"<========");
	        //Log.i("taprest is =====> ",tapRect+"<========");
	        //Log.i("boolean is =====> ",RectF.intersects(linkRect, tapRect)+"<========");
	        //Log.i("link =====> ",link+"<========");
	        
	        
	        
	        if(RectF.intersects(linkRect, tapRect))
	        {
	        	//base.getContext().startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse(link.url)));
	        	
	        	//Log.i("link url is =====> ",link.url+"<========");
	        	if(link!=null && link.url!=null && link.url!="")
	        	{
	        		/*
	        		Intent intent = new Intent(Intent.ACTION_VIEW,Uri.parse(link.url));
	        		Intent chooser = Intent.createChooser(intent, "Open with");
	        		base.getContext().startActivity(chooser);
	        		*/
	        		base.getContext().startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse(link.url)));
	        		return false;
	        	}

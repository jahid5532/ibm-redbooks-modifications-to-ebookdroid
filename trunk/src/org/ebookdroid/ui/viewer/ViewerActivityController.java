...
public class ViewerActivityController extends AbstractActivityController<ViewerActivity> implements
    ...
    public static IActivityController base;
    ...
    public static int maxpagenumberis;
    public static int curpagenumberis;
    ...
    public void afterCreate(final ViewerActivity activity, final boolean recreated) {
        ...
        //Log.i("Is file directory =====> ",(file.isDirectory()+""));
       // Log.i("Show document path is =====> ",(Uri.fromFile(file)+""));
    ...
    public final void doSearch(final ActionEx action) {
    	ViewerActivity.fromcancelbutton=false;
        ...
        /*
        //ViewerActivity.lytContainer.setAlpha(0.3f);
    ...
    @ActionMethod(ids =R.id.actions_cancelSearch)
    public final void doCancelSearch(final ActionEx action) 
    {
    	try
    	{
    		ViewerActivity.fromcancelbutton=true;
	    	SearchControls.m_edit.setText("");
	        final Editable value = action.getParameter("input");
	        final String newPattern = (value != null ? value.toString() : LengthUtils.toString(action.getParameter("text")));
	        final String oldPattern = currentSearchPattern;
	        /*
    	...
    	//((InputMethodManager)getContext().getSystemService(Context.INPUT_METHOD_SERVICE)).toggleSoftInput(0, InputMethodManager.HIDE_IMPLICIT_ONLY);
    	//((InputMethodManager)getContext().getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(SearchControls.m_edit.getWindowToken(), 0);
    	  getActivity().getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
    }
    

    @ActionMethod(ids = R.id.dummyid)
    public void aaaaa(final ActionEx action)
    {
    	dddddd(this);
    }
    
    public void dddddd(IActivityController base) 
    {
    	this.base=base;
    	//BookSettings bs = base.getBookSettings();
        //int offset = bs != null ? bs.firstPageOffset : 1;
        final DocumentModel dm = base.getDocumentModel();
        final Page lastPage = dm != null ? dm.getLastPageObject() : null;
        curpagenumberis = dm != null ? dm.getCurrentViewPageIndex() : 0;
        maxpagenumberis = lastPage != null ? lastPage.index.viewIndex : 0;
        
        //Log.i("offset =====>",offset+"<=====");
        //Log.i("current =====>",curpagenumberis+"<=====");
        //Log.i("max =====>",maxpagenumberis+"<=====");
        
        
        try
        {
        	//Log.i("came to DDDDDD =============> ","method<===============");
        	//ViewerActivity.livedraggingslider.setMax(maxpagenumberis+1);
        	//ViewerActivity.pagenumbercustomlabel.setText(Integer.toString(curpagenumberis+1)+"/"+Integer.toString(maxpagenumberis+1));
        }
        catch(Exception e)
        {
    ...
    public void addBookmark(final ActionEx action) {
        ...
        {
        	for (int i = 0; i < bookSettings.bookmarks.size(); i++)
        	{
        		Bookmark existingbookmarkname=(Bookmark)bookSettings.bookmarks.get(i);
        		if(name.equals(existingbookmarkname.name))
        		{
        			proceedwithadd=false;
        			break;
        		}
        		existingbookmarkname=null;
			}
        	if(proceedwithadd)
        	{
		        final Page page = documentModel.getCurrentPageObject();
		        if (page != null) 
		        {
		            ...
		            state.release();
		            Toast.makeText(getContext(), "Bookmark saved successfully.",Toast.LENGTH_LONG).show();
		        }
        	}
        	else
        	{
        		Toast.makeText(getContext(), "Bookmark name already exists.",Toast.LENGTH_LONG).show();
        	}
        }
        else
        {
        	Toast.makeText(getContext(), "Bookmark name can not be empty.",Toast.LENGTH_LONG).show();
        }
    }
    ...
    public final boolean dispatchKeyEvent(final KeyEvent event) 
    {
        ...
        if (action == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_BACK) 
        {
        	//Log.i("iam here in viwwer act controller ====> ","1<=============");
        	...
        	if(ViewerActivity.isTempFolder) 
        	{
        		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(getManagedComponent());
        ...
        protected void onPostExecute(Throwable result) {
            ...
            try {
                ...
                if (result instanceof MuPdfPasswordException) {
                    ...
                    if(msg.contains("cannot load document"))
                    {
                    	msg="Files has not been dowloaded. Please try again.";
                    }
    ...
    final class SearchTask extends AsyncTask<String, String, RectF> implements SearchModel.ProgressCallback,
        ...
        public void searchFinished(final int pageIndex) {
            ...
            //((InputMethodManager) getContext().getSystemService(Activity.INPUT_METHOD_SERVICE)).toggleSoftInput(0, InputMethodManager.HIDE_IMPLICIT_ONLY);
        	//getActivity().getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
            //sc.m_edit.clearFocus();
        	//sc.m_nextButton.requestFocus();
        ...
        protected void onPostExecute(final RectF result) {
            ...
            if (result != null) {
                ...
                final int controlsHeight = 3 + sc.getActualHeight() + (60+ViewerActivity.heightoftheheader);
            	...
            	//InputMethodManager keyboard = (InputMethodManager) getContext().getSystemService(Context.INPUT_METHOD_SERVICE); keyboard.showSoftInput(sc.m_edit, 0);
            	if(!ViewerActivity.fromcancelbutton)
            ...
            //sc.m_edit.clearFocus();
        	//sc.m_nextButton.requestFocus();
            if(ViewerActivity.leybrdcounter<=0)
            {
            	ViewerActivity.leybrdcounter=ViewerActivity.leybrdcounter+1;
            	((InputMethodManager) getContext().getSystemService(Context.INPUT_METHOD_SERVICE)).toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
                //((InputMethodManager) getContext().getSystemService(Activity.INPUT_METHOD_SERVICE)).toggleSoftInput(0, InputMethodManager.HIDE_IMPLICIT_ONLY);
            	//getActivity().getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
            }
            //ViewerActivity.searchControls.setAlpha(0.6f);
            /*
            if (Build.VERSION.SDK_INT < 11)
            //if(true)
            {
